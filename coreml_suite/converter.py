...
def get_unet(model_type: ModelVersion, ref_pipe):
    ...
    # Keep parameter/buffer dtypes aligned with the fp16 inputs we feed during
    # tracing. When weights remain in fp32 but inputs are cast to fp16 inside the
    # ANE wrapper, PyTorch conv kernels complain about mixed bias/input dtypes.
    # Converting the model to fp16 when the wrapper has been configured to cast
    # inputs avoids those runtime errors and matches the eventual Core ML target.
