...
class UNet2DConditionModelXLANE(UNet2DConditionModelXL):
    ...
    def forward(
        ...
        *additional_residuals: torch.Tensor,
        ...
        # the graph avoids Core ML inserting GPU-only casts. Cast to the model's
        # current dtype rather than assuming float16 so weights and inputs stay
        # aligned even if the model has been re-typed upstream.
