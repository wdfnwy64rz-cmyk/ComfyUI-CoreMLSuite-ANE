...
def _maybe_create_text_time_modules(model: UNet2DConditionModelXL) -> None:
    """Ensure SDXL "text_time" embeddings exist when configs request them.

    Some SDXL checkpoints omit ``add_time_proj`` and ``add_embedding`` because the
    reference configuration may not populate ``addition_embed_type``. When those
    attributes are missing the base ``forward`` fails. This helper reconstructs
    the modules using the model configuration so TorchScript tracing can proceed
    without relying on the checkpoint layout.
    """
    ...
    if addition_time_embed_dim is not None and not hasattr(model, "add_time_proj"):
    ...
    if (
        addition_time_embed_dim is not None
        and projection_dim is not None
        and not hasattr(model, "add_embedding")
    ):
        base_channels = None
